...
export class ProjectService {
  ...
  async createProject(body: CreateProjectDto) {
    ...
    for (const emp of employees) {
      ...
      const mappedModules = emp.assignedModules.map((assigned) => {
        ...
        if (!moduleObjectId) {
          throw new Error(
            `Module ${assigned.moduleId} not found in module list`,
          );
        }
  ...
  async getProjectWithEmployees(projectId: Types.ObjectId) {
    ...
    const modules = await this.moduleModel.find({ projectId: projectId });
    const kanbanStages = await this.kanbanStageModel.find({
      projectId: projectId,
    });
    const fullProject = { ...project.toObject(), modules, kanbanStages };
    // Populate the employees field with full employee data
    return fullProject;
  }
  ...
  async updateProject(projectId: Types.ObjectId, body: CreateProjectDto) {
    const project = await this.projectModel
      .findByIdAndUpdate({ _id: projectId }, body, { new: true })
      .exec();
    return project;
  }
